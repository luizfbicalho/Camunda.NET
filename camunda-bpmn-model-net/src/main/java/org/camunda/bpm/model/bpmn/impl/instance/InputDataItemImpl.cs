...
//	import static org.camunda.bpm.model.bpmn.impl.BpmnModelConstants.BPMN_ELEMENT_INPUT_DATA_ITEM;
	...
	public class InputDataItemImpl : DataInputImpl, InputDataItem
	{
	  ...
	  {
		ModelElementTypeBuilder typeBuilder = modelBuilder.defineType(typeof(InputDataItem), BPMN_ELEMENT_INPUT_DATA_ITEM).namespaceUri(BPMN20_NS).extendsType(typeof(DataInput)).instanceProvider(new ModelTypeInstanceProviderAnonymousInnerClass());

		typeBuilder.build();
	  }

	  private class ModelTypeInstanceProviderAnonymousInnerClass : ModelElementTypeBuilder.ModelTypeInstanceProvider<InputDataItem>
	  {
