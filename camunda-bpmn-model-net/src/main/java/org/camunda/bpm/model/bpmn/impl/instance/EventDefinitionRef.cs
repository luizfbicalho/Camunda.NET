...
//	import static org.camunda.bpm.model.bpmn.impl.BpmnModelConstants.BPMN_ELEMENT_EVENT_DEFINITION_REF;
	...
	public class EventDefinitionRef : BpmnModelElementInstanceImpl
	{
	  ...
	  {
		ModelElementTypeBuilder typeBuilder = modelBuilder.defineType(typeof(EventDefinitionRef), BPMN_ELEMENT_EVENT_DEFINITION_REF).namespaceUri(BPMN20_NS).instanceProvider(new ModelTypeInstanceProviderAnonymousInnerClass());

		typeBuilder.build();
	  }

	  private class ModelTypeInstanceProviderAnonymousInnerClass : ModelElementTypeBuilder.ModelTypeInstanceProvider<EventDefinitionRef>
	  {
