...
//	import static org.camunda.bpm.model.bpmn.impl.BpmnModelConstants.BPMN_ELEMENT_POTENTIAL_OWNER;
	...
	/// </summary>
	public class PotentialOwnerImpl : HumanPerformerImpl, PotentialOwner
	{

	  public static void registerType(ModelBuilder modelBuilder)
	  {
		ModelElementTypeBuilder typeBuilder = modelBuilder.defineType(typeof(PotentialOwner), BPMN_ELEMENT_POTENTIAL_OWNER).namespaceUri(BPMN20_NS).extendsType(typeof(HumanPerformer)).instanceProvider(new ModelTypeInstanceProviderAnonymousInnerClass());
		typeBuilder.build();
	  }

	  private class ModelTypeInstanceProviderAnonymousInnerClass : ModelElementTypeBuilder.ModelTypeInstanceProvider<PotentialOwner>
	  {
