...
namespace org.camunda.bpm.engine.migration
{
	...
	/// </summary>
	public interface MigrationInstructionBuilder : MigrationPlanBuilder
	{

	  /// <summary>
	  /// <para>If the current instruction maps between event-receiving flow nodes that rely on a persistent
	  /// event trigger, this method can be used to determine whether the event trigger should be
	  /// updated during migration
	  /// 
	  /// </para>
	  /// <para>For example, when mapping a message catch event waiting for message <i>A</i>
	  ...
	  /// 
	  /// </para>
	  /// <para>Event-receiving flow nodes are:
	  /// <ul>
	  ...
	  /// 
	  /// </para>
	  /// <para>For other flow nodes, this option must not be used and if so, results in a validation exception
