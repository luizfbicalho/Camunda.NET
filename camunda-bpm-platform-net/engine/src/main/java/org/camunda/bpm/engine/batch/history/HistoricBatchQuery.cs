/*
 * Copyright Camunda Services GmbH and/or licensed to Camunda Services GmbH
 * under one or more contributor license agreements. See the NOTICE file
 * distributed with this work for additional information regarding copyright
 * ownership. Camunda licenses this file to you under the Apache License,
 * Version 2.0; you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
namespace org.camunda.bpm.engine.batch.history
{
	using Query = org.camunda.bpm.engine.query.Query;

	/// <summary>
	/// Query for <seealso cref="HistoricBatch"/> instances.
	/// </summary>
	public interface HistoricBatchQuery : Query<HistoricBatchQuery, HistoricBatch>
	{

	  /// <summary>
	  /// Only select historic batch instances for the given batch id.
	  /// </summary>
	  HistoricBatchQuery batchId(string batchId);

	  /// <summary>
	  /// Only select historic batches of the given type.
	  /// </summary>
	  HistoricBatchQuery type(string type);

	  /// <summary>
	  /// Only select historic batches which are completed or not.
	  /// </summary>
	  HistoricBatchQuery completed(bool completed);

	  /// <summary>
	  /// Only selects historic batches with one of the given tenant ids. </summary>
	  HistoricBatchQuery tenantIdIn(params string[] tenantIds);

	  /// <summary>
	  /// Only selects historic batches which have no tenant id. </summary>
	  HistoricBatchQuery withoutTenantId();

	  /// <summary>
	  /// Returns historic batches sorted by id; must be followed by an invocation of <seealso cref="#asc()"/> or <seealso cref="#desc()"/>.
	  /// </summary>
	  HistoricBatchQuery orderById();

	  /// <summary>
	  /// Returns historic batches sorted by start time; must be followed by an invocation of <seealso cref="#asc()"/> or <seealso cref="#desc()"/>.
	  /// </summary>
	  HistoricBatchQuery orderByStartTime();

	  /// <summary>
	  /// Returns historic batches sorted by end time; must be followed by an invocation of <seealso cref="#asc()"/> or <seealso cref="#desc()"/>.
	  /// </summary>
	  HistoricBatchQuery orderByEndTime();

	  /// <summary>
	  /// Returns historic batches sorted by tenant id; must be followed by an invocation of <seealso cref="#asc()"/> or <seealso cref="#desc()"/>.
	  /// </summary>
	  HistoricBatchQuery orderByTenantId();


	}

}