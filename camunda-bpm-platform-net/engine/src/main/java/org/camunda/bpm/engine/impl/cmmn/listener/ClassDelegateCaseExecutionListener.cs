...
//	import static org.camunda.bpm.engine.impl.util.ClassDelegateUtil.instantiateDelegate;
	...
	/// 
	/// </summary>
	public class ClassDelegateCaseExecutionListener : ClassDelegate, CaseExecutionListener
	{

	  protected internal static readonly CmmnBehaviorLogger LOG = ProcessEngineLogger.CMNN_BEHAVIOR_LOGGER;

	  public ClassDelegateCaseExecutionListener(string className, IList<FieldDeclaration> fieldDeclarations) : base(className, fieldDeclarations)
	  {
	  }

	  public ClassDelegateCaseExecutionListener(Type clazz, IList<FieldDeclaration> fieldDeclarations) : base(clazz, fieldDeclarations)
	  {
	  }
...
//ORIGINAL LINE: public void notify(org.camunda.bpm.engine.delegate.DelegateCaseExecution caseExecution) throws Exception
	  ...
	  protected internal virtual CaseExecutionListener ListenerInstance
	  {
		  get
		  {
			object delegateInstance = instantiateDelegate(className, fieldDeclarations);
			if (delegateInstance is CaseExecutionListener)
			{
	...
	//JAVA TO C# CONVERTER WARNING: The .NET Type.FullName property will not always yield results identical to the Java Class.getName method:
			  throw LOG.missingDelegateParentClassException(delegateInstance.GetType().FullName, typeof(CaseExecutionListener).FullName);
			}
		  }
	  }
