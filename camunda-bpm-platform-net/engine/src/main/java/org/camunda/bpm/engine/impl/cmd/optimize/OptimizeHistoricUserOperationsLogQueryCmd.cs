...
namespace org.camunda.bpm.engine.impl.cmd.optimize
{
	...
	public class OptimizeHistoricUserOperationsLogQueryCmd : Command<IList<UserOperationLogEntry>>
	{
	  ...
	  public OptimizeHistoricUserOperationsLogQueryCmd(DateTime occurredAfter, DateTime occurredAt, int maxResults)
	  {
	  ...
	  public virtual IList<UserOperationLogEntry> execute(CommandContext commandContext)
	  {
		return commandContext.OptimizeManager.getHistoricUserOperationLogs(occurredAfter, occurredAt, maxResults);
	  }
