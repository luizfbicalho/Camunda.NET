...
namespace org.camunda.bpm.engine.impl.management
{
	...
	/// </summary>
	public class DatabasePurgeReport : PurgeReporting<long>
	{
	  ...
	  {
		deletedEntities[key] = value.Value;
	  }
	  ...
	  public virtual string PurgeReportAsString
	  {
		  get
		  {
			...
			foreach (string key in deletedEntities.Keys)
			{
			  builder.Append("Table: ").Append(key).Append(" contains: ").Append(getReportValue(key)).Append(" rows\n");
			}
