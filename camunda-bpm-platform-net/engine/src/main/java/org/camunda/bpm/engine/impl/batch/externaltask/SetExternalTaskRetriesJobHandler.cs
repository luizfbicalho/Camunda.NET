...
namespace org.camunda.bpm.engine.impl.batch.externaltask
{
	...
	public class SetExternalTaskRetriesJobHandler : AbstractBatchJobHandler<SetRetriesBatchConfiguration>
	{

	  public static readonly BatchJobDeclaration JOB_DECLARATION = new BatchJobDeclaration(org.camunda.bpm.engine.batch.Batch_Fields.TYPE_SET_EXTERNAL_TASK_RETRIES);

	  public override string Type
	  {
		  get
		  {
			return org.camunda.bpm.engine.batch.Batch_Fields.TYPE_SET_EXTERNAL_TASK_RETRIES;
		  }
	  }
	  ...
	  public override void execute(BatchJobConfiguration configuration, ExecutionEntity execution, CommandContext commandContext, string tenantId)
	  {
		...
		try
		{
		  commandContext.ProcessEngineConfiguration.ExternalTaskService.setRetries(batchConfiguration.Ids, batchConfiguration.Retries);
		}
		finally
		{
	  ...
	  protected internal override SetRetriesBatchConfiguration createJobConfiguration(SetRetriesBatchConfiguration configuration, IList<string> processIdsForJob)
	  {
		return new SetRetriesBatchConfiguration(processIdsForJob, configuration.Retries);
	  }

	  protected internal override SetExternalTaskRetriesBatchConfigurationJsonConverter JsonConverterInstance
	  {
		  get
		  {
