...
//ORIGINAL LINE: @Test @Deployment(resources = "org/camunda/bpm/engine/test/api/authorization/oneIncidentProcess.bpmn20.xml") public void testMigrate()
	  ...
	  public virtual void testMigrate()
	  {
		...
		{
		  ProcessInstance processInstanceAfterMigration = engineRule.RuntimeService.createProcessInstanceQuery().singleResult();

		  Assert.assertEquals(targetDefinition.Id, processInstanceAfterMigration.ProcessDefinitionId);
		}
	  }
