...
//ORIGINAL LINE: @Parameters(name = "Scenario {index}") public static java.util.Collection<org.camunda.bpm.engine.test.api.authorization.util.AuthorizationScenario[]> scenarios()
	  ...
	  {
		return AuthorizationTestRule.asParameters(scenario().withoutAuthorizations().failsDueToRequired(grant(Resources.BATCH, "batchId", "userId", Permissions.DELETE_HISTORY)), scenario().withAuthorizations(grant(Resources.BATCH, "batchId", "userId", Permissions.DELETE_HISTORY)).succeeds());
	  }
...
//ORIGINAL LINE: @Test public void testDeleteBatch()
	  public virtual void testDeleteBatch()
	  {
		...
		{
		  assertEquals(0, engineRule.HistoryService.createHistoricBatchQuery().count());

		  IList<UserOperationLogEntry> userOperationLogEntries = engineRule.HistoryService.createUserOperationLogQuery().operationType(OPERATION_TYPE_DELETE_HISTORY).list();
