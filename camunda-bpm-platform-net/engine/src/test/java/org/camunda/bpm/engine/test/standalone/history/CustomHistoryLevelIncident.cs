...
namespace org.camunda.bpm.engine.test.standalone.history
{
	...
	public class CustomHistoryLevelIncident : HistoryLevel
	{
	  ...
	  public CustomHistoryLevelIncident(IList<HistoryEventTypes> eventTypes)
	  {
	  ...
	  public virtual bool isHistoryEventProduced(HistoryEventType eventType, object entity)
	  {
		...
		if (eventType.Equals(HistoryEventTypes.BATCH_START) || eventType.Equals(HistoryEventTypes.BATCH_END) || eventType.Equals(HistoryEventTypes.TASK_INSTANCE_CREATE))
		{
		  return true;
		}
		return false;
