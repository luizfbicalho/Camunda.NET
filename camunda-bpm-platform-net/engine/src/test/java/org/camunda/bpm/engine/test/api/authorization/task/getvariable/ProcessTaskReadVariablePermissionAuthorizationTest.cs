...
//ORIGINAL LINE: @RunWith(Parameterized.class) public class ProcessTaskReadVariablePermissionAuthorizationTest extends ProcessTaskAuthorizationTest
	public class ProcessTaskReadVariablePermissionAuthorizationTest : ProcessTaskAuthorizationTest
	{
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: @Parameters(name = "Scenario {index}") public static java.util.Collection<org.camunda.bpm.engine.test.api.authorization.util.AuthorizationScenario[]> scenarios()
		...
		{
		return AuthorizationTestRule.asParameters(scenario().withoutAuthorizations().failsDueToRequired(grant(TASK, "taskId", userId, READ_VARIABLE), grant(PROCESS_DEFINITION, PROCESS_KEY, userId, READ_TASK_VARIABLE)), scenario().withAuthorizations(grant(TASK, "taskId", userId, READ_VARIABLE)), scenario().withAuthorizations(grant(TASK, "*", userId, READ_VARIABLE)), scenario().withAuthorizations(grant(PROCESS_DEFINITION, PROCESS_KEY, userId, READ_TASK_VARIABLE)), scenario().withAuthorizations(grant(PROCESS_DEFINITION, "*", userId, READ_TASK_VARIABLE)).succeeds());
		}
