...
//ORIGINAL LINE: @RequiredHistoryLevel(ProcessEngineConfiguration.HISTORY_FULL) @RunWith(Parameterized.class) public class BatchRestartAuthorizationTest
...
//ORIGINAL LINE: @Test public void executeBatch()
	  public virtual void executeBatch()
	  {
		...
		authRule.init(scenario).withUser("userId").bindResource("processInstance1", processInstance1.Id).bindResource("restartedProcessInstance", "*").bindResource("processInstance2", processInstance2.Id).bindResource("processDefinition", "Process").bindResource("batchId", "*").start();

		Batch batch = engineRule.RuntimeService.restartProcessInstances(processDefinition.Id).processInstanceIds(processInstance1.Id, processInstance2.Id).startAfterActivity("userTask1").executeAsync();
