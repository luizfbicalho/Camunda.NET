...
//	import static org.junit.Assert.assertThat;
	...
	/// </summary>
	public class HistoryCleanupSchedulerIncidentsTest : AbstractHistoryCleanupSchedulerTest
	{
		private bool InstanceFieldsInitialized = false;

		public HistoryCleanupSchedulerIncidentsTest()
		{
		...
		private void InitializeInstanceFields()
		{
			...
			PROCESS = Bpmn.createExecutableProcess(PROCESS_KEY).camundaHistoryTimeToLive(5).startEvent().scriptTask().camundaAsyncBefore().scriptFormat("groovy").scriptText("if(execution.getIncidents().size() == 0) throw new RuntimeException(\"I'm supposed to fail!\")").scriptTask().camundaAsyncBefore().scriptFormat("groovy").scriptText("if(execution.getIncidents().size() == 0) throw new RuntimeException(\"I'm supposed to fail!\")").scriptTask().camundaAsyncBefore().scriptFormat("groovy").scriptText("if(execution.getIncidents().size() == 0) throw new RuntimeException(\"I'm supposed to fail!\")").userTask("userTask").endEvent().done();
		}
...
//ORIGINAL LINE: @Test public void shouldScheduleToNow()
	  public virtual void shouldScheduleToNow()
	  {
		...
		engineConfiguration.HistoryCleanupBatchSize = 3;
...
//ORIGINAL LINE: @Test public void shouldScheduleToLater()
	  public virtual void shouldScheduleToLater()
	  {
		...
		engineConfiguration.HistoryCleanupBatchSize = 4;
		engineConfiguration.initHistoryCleanup();
