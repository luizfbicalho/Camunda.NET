...
//	import static org.hamcrest.MatcherAssert.assertThat;
	...
	public class GetCompletedHistoricProcessInstancesForOptimizeTest
	{
		...
		public GetCompletedHistoricProcessInstancesForOptimizeTest()
		{
...
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: @Test public void getCompletedHistoricProcessInstances()
	  public virtual void getCompletedHistoricProcessInstances()
	  {
		 // given
		...
		// when
		IList<HistoricProcessInstance> completedHistoricProcessInstances = optimizeService.getCompletedHistoricProcessInstances(pastDate(), null, 10);

		// then
		assertThat(completedHistoricProcessInstances.Count, @is(1));
		assertThatInstanceHasAllImportantInformation(completedHistoricProcessInstances[0]);
	  }
...
//ORIGINAL LINE: @Test public void fishedAfterParameterWorks()
	  public virtual void fishedAfterParameterWorks()
	  {
		...
		// when
		IList<HistoricProcessInstance> completedHistoricProcessInstances = optimizeService.getCompletedHistoricProcessInstances(now, null, 10);
...
//ORIGINAL LINE: @Test public void fishedAtParameterWorks()
	  public virtual void fishedAtParameterWorks()
	  {
		...
		// when
		IList<HistoricProcessInstance> completedHistoricProcessInstances = optimizeService.getCompletedHistoricProcessInstances(null, now, 10);

		// then
		assertThat(completedHistoricProcessInstances.Count, @is(1));
		assertThat(completedHistoricProcessInstances[0].Id, @is(processInstance.Id));
	  }
...
//ORIGINAL LINE: @Test public void fishedAfterAndFinishedAtParameterWorks()
	  public virtual void fishedAfterAndFinishedAtParameterWorks()
	  {
		...
		// when
		IList<HistoricProcessInstance> completedHistoricProcessInstances = optimizeService.getCompletedHistoricProcessInstances(now, now, 10);

		// then
		assertThat(completedHistoricProcessInstances.Count, @is(0));
	  }
...
//ORIGINAL LINE: @Test public void maxResultsParameterWorks()
	  public virtual void maxResultsParameterWorks()
	  {
		...
		// when
		IList<HistoricProcessInstance> completedHistoricProcessInstances = optimizeService.getCompletedHistoricProcessInstances(pastDate(), null, 3);

		// then
		assertThat(completedHistoricProcessInstances.Count, @is(3));
	  }
...
//ORIGINAL LINE: @Test public void resultIsSortedByEndTime()
	  public virtual void resultIsSortedByEndTime()
	  {
		...
		// then
		assertThat(completedHistoricProcessInstances.Count, @is(3));
		assertThat(completedHistoricProcessInstances[0].Id, @is(processInstance1.Id));
		assertThat(completedHistoricProcessInstances[1].Id, @is(processInstance2.Id));
		assertThat(completedHistoricProcessInstances[2].Id, @is(processInstance3.Id));
	  }

//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: @Test public void fetchOnlyCompletedProcessInstances()
	  public virtual void fetchOnlyCompletedProcessInstances()
	  {
		 // given
		...
		// then
		assertThat(completedHistoricProcessInstances.Count, @is(1));
			assertThat(completedHistoricProcessInstances[0].Id, @is(processInstanceToComplete.Id));
	  ...
	  private void assertThatInstanceHasAllImportantInformation(HistoricProcessInstance historicProcessInstance)
	  {
		...
		assertThat(historicProcessInstance.DurationInMillis, notNullValue());
		assertThat(historicProcessInstance.StartTime, notNullValue());
		assertThat(historicProcessInstance.EndTime, notNullValue());
	  }
