...
namespace org.camunda.bpm.engine.test.cmmn.tasklistener.util
{
	...
	public class MyTaskListener
	{
	  ...
	  public virtual void notify(DelegateTask task, string @event)
	  {
		string eventCounterName = @event + "EventCounter";

		int? eventCounter = (int?) task.getVariable(eventCounterName);
		...
		int? counter = (int?) task.getVariable("eventCounter");
