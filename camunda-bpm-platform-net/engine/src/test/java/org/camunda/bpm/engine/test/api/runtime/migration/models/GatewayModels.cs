...
namespace org.camunda.bpm.engine.test.api.runtime.migration.models
{
	...
	public class GatewayModels
	{
	  ...
	  public static readonly BpmnModelInstance PARALLEL_GW = ProcessModels.newModel().startEvent().parallelGateway("fork").userTask("parallel1").parallelGateway("join").moveToNode("fork").userTask("parallel2").connectTo("join").userTask("afterJoin").endEvent().done();
	  ...
	  public static readonly BpmnModelInstance INCLUSIVE_GW = ProcessModels.newModel().startEvent().inclusiveGateway("fork").userTask("parallel1").inclusiveGateway("join").moveToNode("fork").userTask("parallel2").connectTo("join").userTask("afterJoin").endEvent().done();
