...
//	import static org.mockito.Mockito.when;
	...
	public class DecisionRequirementsDefinitionRestServiceQueryTest : AbstractRestServiceTest
	{
...
//ORIGINAL LINE: @ClassRule public static org.camunda.bpm.engine.rest.util.container.TestContainerRule rule = new org.camunda.bpm.engine.rest.util.container.TestContainerRule();
	  ...
	  protected internal static readonly string DECISION_REQUIREMENTS_DEFINITION_QUERY_URL = TEST_RESOURCE_ROOT_PATH + "/decision-requirements-definition";
	  protected internal static readonly string DECISION_REQUIREMENTS_DEFINITION_COUNT_QUERY_URL = DECISION_REQUIREMENTS_DEFINITION_QUERY_URL + "/count";
...
//ORIGINAL LINE: @Test public void testInvalidBooleanParameter()
	  public virtual void testInvalidBooleanParameter()
	  {
		...
		given().queryParam("latestVersion", anInvalidBooleanQueryParam).then().expect().statusCode(Status.BAD_REQUEST.StatusCode).contentType(ContentType.JSON).body("type", equalTo(typeof(InvalidRequestException).Name)).body("message", equalTo("Cannot set query parameter 'latestVersion' to value 'neitherTrueNorFalse': " + "Cannot convert value neitherTrueNorFalse to java type java.lang.Boolean")).when().get(DECISION_REQUIREMENTS_DEFINITION_QUERY_URL);
	  }
...
//ORIGINAL LINE: @Test public void testSortByParameterOnly()
	  ...
	  {
		given().queryParam("sortBy", "id").then().expect().statusCode(Status.BAD_REQUEST.StatusCode).when().get(DECISION_REQUIREMENTS_DEFINITION_QUERY_URL);
	  }
...
//ORIGINAL LINE: @Test public void testSortOrderParameterOnly()
	  ...
	  {
		given().queryParam("sortOrder", "asc").then().expect().statusCode(Status.BAD_REQUEST.StatusCode).when().get(DECISION_REQUIREMENTS_DEFINITION_QUERY_URL);
	  }

	  protected internal virtual void executeAndVerifySorting(string sortBy, string sortOrder, Status expectedStatus)
	  {
		given().queryParam("sortBy", sortBy).queryParam("sortOrder", sortOrder).then().expect().statusCode(expectedStatus.StatusCode).when().get(DECISION_REQUIREMENTS_DEFINITION_QUERY_URL);
	  }
...
//ORIGINAL LINE: @Test public void testSuccessfulPagination()
	  public virtual void testSuccessfulPagination()
	  {
		...
		given().queryParam("firstResult", firstResult).queryParam("maxResults", maxResults).then().expect().statusCode(Status.OK.StatusCode).when().get(DECISION_REQUIREMENTS_DEFINITION_QUERY_URL);
...
//ORIGINAL LINE: @Test public void testMissingFirstResultParameter()
	  public virtual void testMissingFirstResultParameter()
	  {
		...
		given().queryParam("maxResults", maxResults).then().expect().statusCode(Status.OK.StatusCode).when().get(DECISION_REQUIREMENTS_DEFINITION_QUERY_URL);
...
//ORIGINAL LINE: @Test public void testMissingMaxResultsParameter()
	  public virtual void testMissingMaxResultsParameter()
	  {
		...
		given().queryParam("firstResult", firstResult).then().expect().statusCode(Status.OK.StatusCode).when().get(DECISION_REQUIREMENTS_DEFINITION_QUERY_URL);
...
//ORIGINAL LINE: @Test public void testAdditionalParameters()
	  public virtual void testAdditionalParameters()
	  {
		...
		given().queryParams(queryParameters).expect().statusCode(Status.OK.StatusCode).when().get(DECISION_REQUIREMENTS_DEFINITION_QUERY_URL);

		// assert query invocation
		verify(mockedQuery).decisionRequirementsDefinitionId(queryParameters["decisionRequirementsDefinitionId"]);
		verify(mockedQuery).decisionRequirementsDefinitionCategory(queryParameters["category"]);
		verify(mockedQuery).decisionRequirementsDefinitionCategoryLike(queryParameters["categoryLike"]);
		verify(mockedQuery).decisionRequirementsDefinitionName(queryParameters["name"]);
		verify(mockedQuery).decisionRequirementsDefinitionNameLike(queryParameters["nameLike"]);
		verify(mockedQuery).decisionRequirementsDefinitionKey(queryParameters["key"]);
		verify(mockedQuery).decisionRequirementsDefinitionKeyLike(queryParameters["keyLike"]);
		verify(mockedQuery).decisionRequirementsDefinitionVersion(int.Parse(queryParameters["version"]));
		verify(mockedQuery).latestVersion();
		verify(mockedQuery).decisionRequirementsDefinitionResourceName(queryParameters["resourceName"]);
		verify(mockedQuery).decisionRequirementsDefinitionResourceNameLike(queryParameters["resourceNameLike"]);
...
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: @Test public void testDecisionRequirementsDefinitionWithoutTenantId()
	  public virtual void testDecisionRequirementsDefinitionWithoutTenantId()
	  {
		Response response = given().queryParam("withoutTenantId", true).then().expect().statusCode(Status.OK.StatusCode).when().get(DECISION_REQUIREMENTS_DEFINITION_QUERY_URL);
...
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: @Test public void testDecisionRequirementsDefinitionTenantIdIncludeDefinitionsWithoutTenantid()
	  public virtual void testDecisionRequirementsDefinitionTenantIdIncludeDefinitionsWithoutTenantid()
	  {
		IList<DecisionRequirementsDefinition> decisionDefinitions = Arrays.asList(MockProvider.mockDecisionRequirementsDefinition().tenantId(null).build(), MockProvider.mockDecisionRequirementsDefinition().tenantId(MockProvider.EXAMPLE_TENANT_ID).build());
		mockedQuery = createMockQuery(decisionDefinitions);

		Response response = given().queryParam("tenantIdIn", MockProvider.EXAMPLE_TENANT_ID).queryParam("includeDecisionRequirementsDefinitionsWithoutTenantId", true).then().expect().statusCode(Status.OK.StatusCode).when().get(DECISION_REQUIREMENTS_DEFINITION_QUERY_URL);
...
//ORIGINAL LINE: @Test public void testQueryCount()
	  ...
	  {
		expect().statusCode(Status.OK.StatusCode).body("count", equalTo(1)).when().get(DECISION_REQUIREMENTS_DEFINITION_COUNT_QUERY_URL);
	  ...
	  private IDictionary<string, string> CompleteQueryParameters
	  {
		  get
		  {
			...
			parameters["decisionRequirementsDefinitionId"] = "anId";
