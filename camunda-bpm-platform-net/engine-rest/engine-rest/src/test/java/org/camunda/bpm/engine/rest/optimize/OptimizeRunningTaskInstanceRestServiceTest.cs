...
//	import static org.mockito.Mockito.when;
	...
	public class OptimizeRunningTaskInstanceRestServiceTest : AbstractRestServiceTest
	{

	  public static readonly string OPTIMIZE_RUNNING_TASK_INSTANCE_PATH = TEST_RESOURCE_ROOT_PATH + "/optimize/task-instance/running";
...
//ORIGINAL LINE: @Test public void testNoQueryParameters()
	  ...
	  {
		given().then().expect().statusCode(Status.OK.StatusCode).contentType(MediaType.APPLICATION_JSON).when().get(OPTIMIZE_RUNNING_TASK_INSTANCE_PATH);

		verify(mockedOptimizeService).getRunningHistoricTaskInstances(null, null, int.MaxValue);
...
//ORIGINAL LINE: @Test public void testStartedAfterQueryParameter()
	  public virtual void testStartedAfterQueryParameter()
	  {
		...
		given().queryParam("startedAfter", DATE_FORMAT_WITH_TIMEZONE.format(now)).then().expect().statusCode(Status.OK.StatusCode).contentType(MediaType.APPLICATION_JSON).when().get(OPTIMIZE_RUNNING_TASK_INSTANCE_PATH);

		verify(mockedOptimizeService).getRunningHistoricTaskInstances(now, null, int.MaxValue);
		verifyNoMoreInteractions(mockedOptimizeService);
	  }
...
//ORIGINAL LINE: @Test public void testMaxResultsQueryParameter()
	  ...
	  {
		given().queryParam("maxResults", 10).then().expect().statusCode(Status.OK.StatusCode).contentType(MediaType.APPLICATION_JSON).when().get(OPTIMIZE_RUNNING_TASK_INSTANCE_PATH);
...
//ORIGINAL LINE: @Test public void testQueryParameterCombination()
	  ...
	  {
		DateTime now = DateTime.Now;
		given().queryParam("startedAfter", DATE_FORMAT_WITH_TIMEZONE.format(now)).queryParam("startedAt", DATE_FORMAT_WITH_TIMEZONE.format(now)).queryParam("maxResults", 10).then().expect().statusCode(Status.OK.StatusCode).contentType(MediaType.APPLICATION_JSON).when().get(OPTIMIZE_RUNNING_TASK_INSTANCE_PATH);
