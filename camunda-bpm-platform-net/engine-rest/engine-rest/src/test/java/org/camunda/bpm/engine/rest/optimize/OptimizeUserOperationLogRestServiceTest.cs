...
//	import static org.mockito.Mockito.when;
	...
	public class OptimizeUserOperationLogRestServiceTest : AbstractRestServiceTest
	{

	  public static readonly string OPTIMIZE_USER_OPERATION_LOG_PATH = TEST_RESOURCE_ROOT_PATH + "/optimize/user-operation";
...
//ORIGINAL LINE: @Test public void testNoQueryParameters()
	  ...
	  {
		given().then().expect().statusCode(Status.OK.StatusCode).contentType(MediaType.APPLICATION_JSON).when().get(OPTIMIZE_USER_OPERATION_LOG_PATH);

		verify(mockedOptimizeService).getHistoricUserOperationLogs(null, null, int.MaxValue);
...
//ORIGINAL LINE: @Test public void testOccurredAfterQueryParameter()
	  public virtual void testOccurredAfterQueryParameter()
	  {
		...
		given().queryParam("occurredAfter", DATE_FORMAT_WITH_TIMEZONE.format(now)).then().expect().statusCode(Status.OK.StatusCode).contentType(MediaType.APPLICATION_JSON).when().get(OPTIMIZE_USER_OPERATION_LOG_PATH);

		verify(mockedOptimizeService).getHistoricUserOperationLogs(now, null, int.MaxValue);
		verifyNoMoreInteractions(mockedOptimizeService);
	  }
...
//ORIGINAL LINE: @Test public void testOccurredAtQueryParameter()
	  ...
	  {
		DateTime now = DateTime.Now;
		given().queryParam("occurredAt", DATE_FORMAT_WITH_TIMEZONE.format(now)).then().expect().statusCode(Status.OK.StatusCode).contentType(MediaType.APPLICATION_JSON).when().get(OPTIMIZE_USER_OPERATION_LOG_PATH);

		verify(mockedOptimizeService).getHistoricUserOperationLogs(null, now, int.MaxValue);
		verifyNoMoreInteractions(mockedOptimizeService);
	  }
...
//ORIGINAL LINE: @Test public void testMaxResultsQueryParameter()
	  ...
	  {
		given().queryParam("maxResults", 10).then().expect().statusCode(Status.OK.StatusCode).contentType(MediaType.APPLICATION_JSON).when().get(OPTIMIZE_USER_OPERATION_LOG_PATH);
