...
//	import static org.mockito.Mockito.when;
	...
	public class OptimizeHistoricIdentityLinkLogRestServiceTest : AbstractRestServiceTest
	{

	  public static readonly string OPTIMIZE_HISTORIC_IDENTITY_LINK_LOG_PATH = TEST_RESOURCE_ROOT_PATH + "/optimize/identity-link-log";
...
//ORIGINAL LINE: @Test public void testNoQueryParameters()
	  ...
	  {
		given().then().expect().statusCode(Status.OK.StatusCode).contentType(MediaType.APPLICATION_JSON).when().get(OPTIMIZE_HISTORIC_IDENTITY_LINK_LOG_PATH);

		verify(mockedOptimizeService).getHistoricIdentityLinkLogs(null, null, int.MaxValue);
...
//ORIGINAL LINE: @Test public void testOccurredAfterQueryParameter()
	  public virtual void testOccurredAfterQueryParameter()
	  {
		...
		given().queryParam("occurredAfter", DATE_FORMAT_WITH_TIMEZONE.format(now)).then().expect().statusCode(Status.OK.StatusCode).contentType(MediaType.APPLICATION_JSON).when().get(OPTIMIZE_HISTORIC_IDENTITY_LINK_LOG_PATH);

		verify(mockedOptimizeService).getHistoricIdentityLinkLogs(now, null, int.MaxValue);
		verifyNoMoreInteractions(mockedOptimizeService);
	  }
...
//ORIGINAL LINE: @Test public void testMaxResultsQueryParameter()
	  ...
	  {
		given().queryParam("maxResults", 10).then().expect().statusCode(Status.OK.StatusCode).contentType(MediaType.APPLICATION_JSON).when().get(OPTIMIZE_HISTORIC_IDENTITY_LINK_LOG_PATH);
...
//ORIGINAL LINE: @Test public void testQueryParameterCombination()
	  ...
	  {
		DateTime now = DateTime.Now;
		given().queryParam("occurredAfter", DATE_FORMAT_WITH_TIMEZONE.format(now)).queryParam("occurredAt", DATE_FORMAT_WITH_TIMEZONE.format(now)).queryParam("maxResults", 10).then().expect().statusCode(Status.OK.StatusCode).contentType(MediaType.APPLICATION_JSON).when().get(OPTIMIZE_HISTORIC_IDENTITY_LINK_LOG_PATH);
