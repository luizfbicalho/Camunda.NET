...
//	import static org.mockito.Mockito.when;
	...
	public class OptimizeCompletedTaskInstanceRestServiceTest : AbstractRestServiceTest
	{

	  public static readonly string OPTIMIZE_COMPLETED_TASK_INSTANCE_PATH = TEST_RESOURCE_ROOT_PATH + "/optimize/task-instance/completed";
...
//ORIGINAL LINE: @Test public void testNoQueryParameters()
	  ...
	  {
		given().then().expect().statusCode(Status.OK.StatusCode).contentType(MediaType.APPLICATION_JSON).when().get(OPTIMIZE_COMPLETED_TASK_INSTANCE_PATH);

		verify(mockedOptimizeService).getCompletedHistoricTaskInstances(null, null, int.MaxValue);
...
//ORIGINAL LINE: @Test public void testFinishedAtQueryParameter()
	  ...
	  {
		DateTime now = DateTime.Now;
		given().queryParam("finishedAt", DATE_FORMAT_WITH_TIMEZONE.format(now)).then().expect().statusCode(Status.OK.StatusCode).contentType(MediaType.APPLICATION_JSON).when().get(OPTIMIZE_COMPLETED_TASK_INSTANCE_PATH);

		verify(mockedOptimizeService).getCompletedHistoricTaskInstances(null, now, int.MaxValue);
		verifyNoMoreInteractions(mockedOptimizeService);
	  }
...
//ORIGINAL LINE: @Test public void testMaxResultsQueryParameter()
	  ...
	  {
		given().queryParam("maxResults", 10).then().expect().statusCode(Status.OK.StatusCode).contentType(MediaType.APPLICATION_JSON).when().get(OPTIMIZE_COMPLETED_TASK_INSTANCE_PATH);
...
//ORIGINAL LINE: @Test public void testQueryParameterCombination()
	  ...
	  {
		DateTime now = DateTime.Now;
		given().queryParam("finishedAfter", DATE_FORMAT_WITH_TIMEZONE.format(now)).queryParam("finishedAt", DATE_FORMAT_WITH_TIMEZONE.format(now)).queryParam("maxResults", 10).then().expect().statusCode(Status.OK.StatusCode).contentType(MediaType.APPLICATION_JSON).when().get(OPTIMIZE_COMPLETED_TASK_INSTANCE_PATH);
