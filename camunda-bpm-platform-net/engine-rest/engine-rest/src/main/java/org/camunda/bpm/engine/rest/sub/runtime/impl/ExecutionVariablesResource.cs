...
namespace org.camunda.bpm.engine.rest.sub.runtime.impl
{
	...
	public class ExecutionVariablesResource : AbstractVariablesResource
	{

	  private string resourceTypeName;

	  public ExecutionVariablesResource(ProcessEngine engine, string resourceId, bool isProcessInstance, ObjectMapper objectMapper) : base(engine, resourceId, objectMapper)
	  {
	  ...
	  {
		engine.RuntimeService.removeVariable(resourceId, variableKey);
	  }

	  protected internal override VariableMap getVariableEntities(bool deserializeValues)
	  {
		return engine.RuntimeService.getVariablesTyped(resourceId, deserializeValues);
	  }

	  protected internal override TypedValue getVariableEntity(string variableKey, bool deserializeValue)
	  {
		return engine.RuntimeService.getVariableTyped(resourceId, variableKey, deserializeValue);
	  }

	  protected internal override void setVariableEntity(string variableKey, TypedValue variableValue)
	  {
		engine.RuntimeService.setVariable(resourceId, variableKey, variableValue);
	  }
