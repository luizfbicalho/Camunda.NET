...
namespace org.camunda.bpm.engine.rest.dto.history.batch
{
	...
	public class CleanableHistoricBatchReportResultDto
	{
	  ...
	  protected internal virtual CleanableHistoricBatchReport createNewReportQuery(ProcessEngine engine)
	  {
		return engine.HistoryService.createCleanableHistoricBatchReport();
	  }
	  public static IList<CleanableHistoricBatchReportResultDto> convert(IList<CleanableHistoricBatchReportResult> reportResult)
	  {
		IList<CleanableHistoricBatchReportResultDto> dtos = new List<CleanableHistoricBatchReportResultDto>();
		foreach (CleanableHistoricBatchReportResult current in reportResult)
		{
		  CleanableHistoricBatchReportResultDto dto = new CleanableHistoricBatchReportResultDto();
		  dto.BatchType = current.BatchType;
		  dto.HistoryTimeToLive = current.HistoryTimeToLive;
		  dto.FinishedBatchesCount = current.FinishedBatchesCount;
		  dto.CleanableBatchesCount = current.CleanableBatchesCount;
		  dtos.Add(dto);
		}
		return dtos;
	  }
