...
namespace org.camunda.bpm.engine.rest.hal.processDefinition
{
	...
	/// 
	/// </summary>
	public class HalProcessDefinitionResolver : HalIdResourceCacheLinkResolver
	{
	  ...
	  protected internal override Type HalResourceClass
	  {
		  get
		  {
			return typeof(HalProcessDefinition);
		  }
	  }
...
//ORIGINAL LINE: protected java.util.List<org.camunda.bpm.engine.rest.hal.HalResource<?>> resolveNotCachedLinks(String[] linkedIds, org.camunda.bpm.engine.ProcessEngine processEngine)
	  protected internal override IList<HalResource<object>> resolveNotCachedLinks(string[] linkedIds, ProcessEngine processEngine)
	  {
		...
		IList<ProcessDefinition> processDefinitions = repositoryService.createProcessDefinitionQuery().processDefinitionIdIn(linkedIds).listPage(0, linkedIds.Length);

//JAVA TO C# CONVERTER WARNING: Java wildcard generics have no direct equivalent in .NET:
//ORIGINAL LINE: java.util.List<org.camunda.bpm.engine.rest.hal.HalResource<?>> resolved = new java.util.ArrayList<org.camunda.bpm.engine.rest.hal.HalResource<?>>();
		IList<HalResource<object>> resolved = new List<HalResource<object>>();
		foreach (ProcessDefinition procDef in processDefinitions)
		{
