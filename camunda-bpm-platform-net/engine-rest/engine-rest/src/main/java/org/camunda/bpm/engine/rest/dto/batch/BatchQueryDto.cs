...
//	import static true;
	...
	public class BatchQueryDto : AbstractQueryDto<BatchQuery>
	{
	  ...
	  public BatchQueryDto(ObjectMapper objectMapper, MultivaluedMap<string, string> queryParameters) : base(objectMapper, queryParameters)
	  {
	  }
	  ...
	  protected internal override BatchQuery createNewQuery(ProcessEngine engine)
	  {
		return engine.ManagementService.createBatchQuery();
	  }

	  protected internal virtual void applyFilters(BatchQuery query)
	  {
	  ...
	  protected internal virtual void applySortBy(BatchQuery query, string sortBy, IDictionary<string, object> parameters, ProcessEngine engine)
	  {
