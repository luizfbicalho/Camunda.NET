...
namespace org.camunda.bpm.engine.rest.exception
{
	...
	public class InvalidRequestException : RestException
	{
	  ...
	  public InvalidRequestException(Status status, string message) : base(status, message)
	  {
	  }

	  public InvalidRequestException(Status status, Exception cause, string message) : base(status, cause, message)
	  {
	  }
	}
}