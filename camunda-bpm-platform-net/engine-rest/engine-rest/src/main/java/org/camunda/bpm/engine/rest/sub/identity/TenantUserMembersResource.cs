...
//ORIGINAL LINE: @PUT @Path("/{userId}") public void createMembership(@PathParam("userId") String userId);
	  void createMembership(string userId);

//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: @DELETE @Path("/{userId}") public void deleteMembership(@PathParam("userId") String userId);
...
//ORIGINAL LINE: @OPTIONS @Produces(javax.ws.rs.core.MediaType.APPLICATION_JSON) org.camunda.bpm.engine.rest.dto.ResourceOptionsDto availableOperations(@Context UriInfo context);
	...
	public static class TenantUserMembersResource_Fields
	{
	  public const string PATH = "/user-members";
	}
