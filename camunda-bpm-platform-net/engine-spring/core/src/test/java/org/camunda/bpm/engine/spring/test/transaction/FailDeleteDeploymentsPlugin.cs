...
namespace org.camunda.bpm.engine.spring.test.transaction
{
	...
	/// 
	/// </summary>
	public class FailDeleteDeploymentsPlugin : ProcessEnginePlugin
	{

	  public virtual void preInit(ProcessEngineConfigurationImpl processEngineConfiguration)
	  {
		processEngineConfiguration.CustomPostCommandInterceptorsTxRequired = Arrays.asList<CommandInterceptor>(new FailDeleteDeploymentCommandInterceptor());
		processEngineConfiguration.CustomPostCommandInterceptorsTxRequiresNew = Arrays.asList<CommandInterceptor>(new FailDeleteDeploymentCommandInterceptor());
	  }
