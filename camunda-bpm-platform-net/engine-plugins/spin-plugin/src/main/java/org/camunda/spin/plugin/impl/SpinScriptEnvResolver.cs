...
namespace org.camunda.spin.plugin.impl
{
	...
	using SpinScriptEnv = org.camunda.spin.scripting.SpinScriptEnv;
	...
	/// 
	/// </summary>
	public class SpinScriptEnvResolver : ScriptEnvResolver
	{

	  public virtual string[] resolve(string language)
	  {
		string envScript = SpinScriptEnv.get(language);
		if (!string.ReferenceEquals(envScript, null))
		{
