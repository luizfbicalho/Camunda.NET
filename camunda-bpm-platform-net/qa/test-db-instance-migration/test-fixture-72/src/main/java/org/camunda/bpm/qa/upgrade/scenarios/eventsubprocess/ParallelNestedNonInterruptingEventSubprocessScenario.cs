...
//ORIGINAL LINE: @Deployment public static String deployProcess()
	  ...
	  [DescribesScenario("init"), Times(6)]
	  ...
	  private class ScenarioSetupAnonymousInnerClass : ScenarioSetup
	  {
		  ...
		  {
			engine.RuntimeService.startProcessInstanceByKey("ParallelNestedNonInterruptingEventSubprocessScenario", scenarioName);

			engine.RuntimeService.createMessageCorrelation("Message").processInstanceBusinessKey(scenarioName).correlate();
		  }
	  }

	  [DescribesScenario("init.innerTask"), ExtendsScenario("init"), Times(6)]
	  ...
	  private class ScenarioSetupAnonymousInnerClass2 : ScenarioSetup
	  {
		  public void execute(ProcessEngine engine, string scenarioName)
		  {
			Task task = engine.TaskService.createTaskQuery().processInstanceBusinessKey(scenarioName).taskDefinitionKey("innerTask").singleResult();
