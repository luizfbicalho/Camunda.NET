...
//ORIGINAL LINE: @Deployment public static String deployProcess()
	  ...
	  private class ScenarioSetupAnonymousInnerClass : ScenarioSetup
	  {
		  ...
		  {
			engine.RuntimeService.startProcessInstanceByKey("TransactionCancelCompensationScenario", scenarioName);
	  ...
	  private class ScenarioSetupAnonymousInnerClass2 : ScenarioSetup
	  {
		  public void execute(ProcessEngine engine, string scenarioName)
		  {
			...
			Task beforeCancelTask = engine.TaskService.createTaskQuery().processInstanceBusinessKey(scenarioName).singleResult();
			engine.TaskService.complete(beforeCancelTask.Id);
		  }
	  }
