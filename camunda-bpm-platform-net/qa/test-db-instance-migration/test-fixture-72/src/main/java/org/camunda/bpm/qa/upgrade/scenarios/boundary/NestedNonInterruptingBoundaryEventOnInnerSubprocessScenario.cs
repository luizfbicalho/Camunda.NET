...
//ORIGINAL LINE: @Deployment public static String deployMessageBoundary()
	  ...
	  private class ScenarioSetupAnonymousInnerClass : ScenarioSetup
	  {
		  ...
		  {
			engine.RuntimeService.startProcessInstanceByKey("NestedNonInterruptingMessageBoundaryEventOnInnerSubprocessScenario", scenarioName);
	  ...
	  private class ScenarioSetupAnonymousInnerClass2 : ScenarioSetup
	  {
		  public void execute(ProcessEngine engine, string scenarioName)
		  {
			ProcessInstance instance = engine.RuntimeService.startProcessInstanceByKey("NestedNonInterruptingTimerBoundaryEventOnInnerSubprocessScenario", scenarioName);
