...
//ORIGINAL LINE: @Deployment public static String deployNestedProcess()
	  ...
	  private class ScenarioSetupAnonymousInnerClass : ScenarioSetup
	  {
		  ...
		  {
			engine.RuntimeService.startProcessInstanceByKey("ParallelScopeTasksScenario.plain", scenarioName);
		  }
	  }
	  ...
	  private class ScenarioSetupAnonymousInnerClass2 : ScenarioSetup
	  {
		  public void execute(ProcessEngine engine, string scenarioName)
		  {
			engine.RuntimeService.startProcessInstanceByKey("ParallelScopeTasksScenario.nested", scenarioName);
		  }
	  }
	}
