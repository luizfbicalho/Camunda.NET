...
//ORIGINAL LINE: @Deployment public static String deployAsyncBeforeSubprocessProcess()
...
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: @Deployment public static String deployAsyncBeforeTaskProcess()
	  ...
	  private class ScenarioSetupAnonymousInnerClass : ScenarioSetup
	  {
		  ...
		  {
			engine.RuntimeService.startProcessInstanceByKey("AsyncBeforeParallelMultiInstanceSubprocess", scenarioName);
		  }
	  }
	  ...
	  private class ScenarioSetupAnonymousInnerClass2 : ScenarioSetup
	  {
		  public void execute(ProcessEngine engine, string scenarioName)
		  {
			engine.RuntimeService.startProcessInstanceByKey("AsyncBeforeParallelMultiInstanceTask", scenarioName);
		  }
	  }
	}
