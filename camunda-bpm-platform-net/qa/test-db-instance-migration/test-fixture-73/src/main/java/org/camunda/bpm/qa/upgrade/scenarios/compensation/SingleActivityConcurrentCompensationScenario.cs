...
//ORIGINAL LINE: @Deployment public static String deployProcess()
	  ...
	  private class ScenarioSetupAnonymousInnerClass : ScenarioSetup
	  {
		  ...
		  {
			engine.RuntimeService.startProcessInstanceByKey("SingleActivityConcurrentCompensationScenario", scenarioName);
	  ...
	  private class ScenarioSetupAnonymousInnerClass2 : ScenarioSetup
	  {
		  public void execute(ProcessEngine engine, string scenarioName)
		  {
			...
			Task beforeCompensateTask = engine.TaskService.createTaskQuery().taskDefinitionKey("beforeCompensate").processInstanceBusinessKey(scenarioName).singleResult();
