...
//ORIGINAL LINE: @Deployment public static String deployProcess()
	  ...
	  [DescribesScenario("init.throwCompensate"), Times(4)]
	  ...
	  private class ScenarioSetupAnonymousInnerClass : ScenarioSetup
	  {
		  ...
		  {
			engine.RuntimeService.startProcessInstanceByKey("InterruptingEventSubProcessNestedCompensationScenario", scenarioName);

			// trigger the event subprocess
			engine.RuntimeService.correlateMessage("EventSubProcessMessage", scenarioName);

			// complete the task to compensate and then throw compensation
			Task innerSubProcessTask = engine.TaskService.createTaskQuery().processInstanceBusinessKey(scenarioName).singleResult();
			engine.TaskService.complete(innerSubProcessTask.Id);
		  }
	  }
	}
