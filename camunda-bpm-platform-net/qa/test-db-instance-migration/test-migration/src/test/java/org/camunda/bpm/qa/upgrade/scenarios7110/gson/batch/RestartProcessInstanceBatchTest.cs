...
//	import static org.junit.Assert.assertThat;
	...
	/// </summary>
	[ScenarioUnderTest("RestartProcessInstanceBatchScenario"), Origin("7.11.0")]
...
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: @ScenarioUnderTest("initRestartProcessInstanceBatch.1") @Test public void testRestartProcessInstanceBatch()
	  [ScenarioUnderTest("initRestartProcessInstanceBatch.1")]
	  public virtual void testRestartProcessInstanceBatch()
	  {
		IList<ProcessInstance> processInstances = engineRule.RuntimeService.createProcessInstanceQuery().processDefinitionKey("oneTaskProcessRestart_710").processInstanceBusinessKey("RestartProcessInstanceBatchScenario").list();

		// assume
		assertThat(processInstances.Count, @is(0));

		Batch batch = engineRule.ManagementService.createBatchQuery().type(org.camunda.bpm.engine.batch.Batch_Fields.TYPE_PROCESS_INSTANCE_RESTART).singleResult();
		...
		processInstances = engineRule.RuntimeService.createProcessInstanceQuery().processDefinitionKey("oneTaskProcessRestart_710").list();
