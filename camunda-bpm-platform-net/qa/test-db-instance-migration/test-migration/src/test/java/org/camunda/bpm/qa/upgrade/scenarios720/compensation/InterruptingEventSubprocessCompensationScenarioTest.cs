...
//	import static org.camunda.bpm.qa.upgrade.util.ActivityInstanceAssert.describeActivityInstanceTree;
	...
	/// 
	/// </summary>
	[ScenarioUnderTest("InterruptingEventSubprocessCompensationScenario"), Origin("7.2.0")]
...
//ORIGINAL LINE: @Test @ScenarioUnderTest("init.3") public void testInitActivityInstanceTree()
	  ...
	  public virtual void testInitActivityInstanceTree()
	  {
		...
		assertThat(activityInstance).hasStructure(describeActivityInstanceTree(instance.ProcessDefinitionId).beginScope("subProcess").beginScope("throwCompensate").activity("undoTask").done());
	  }
...
//ORIGINAL LINE: @Test @ScenarioUnderTest("init.triggerCompensation.3") public void testInitTriggerCompensationActivityInstanceTree()
	  ...
	  public virtual void testInitTriggerCompensationActivityInstanceTree()
	  {
		...
		ActivityInstance activityInstance = rule.RuntimeService.getActivityInstance(instance.Id);
		Assert.assertNotNull(activityInstance);
		assertThat(activityInstance).hasStructure(describeActivityInstanceTree(instance.ProcessDefinitionId).beginScope("subProcess").activity("throwCompensate").activity("undoTask").done());
	  }
	}
