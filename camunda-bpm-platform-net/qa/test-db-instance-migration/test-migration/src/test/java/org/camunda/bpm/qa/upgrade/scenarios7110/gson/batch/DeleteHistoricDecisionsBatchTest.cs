...
//	import static org.junit.Assert.assertThat;
	...
	/// </summary>
	[ScenarioUnderTest("DeleteHistoricDecisionsBatchScenario"), Origin("7.11.0")]
...
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: @ScenarioUnderTest("initDeleteHistoricDecisionsBatch.1") @Test public void testDeleteHistoricDecisionsBatch()
	  [ScenarioUnderTest("initDeleteHistoricDecisionsBatch.1")]
	  public virtual void testDeleteHistoricDecisionsBatch()
	  {
		IList<HistoricDecisionInstance> historicDecisionInstances = engineRule.HistoryService.createHistoricDecisionInstanceQuery().decisionDefinitionKey("decision_710").list();

		// assume
		assertThat(historicDecisionInstances.Count, @is(10));

		Batch batch = engineRule.ManagementService.createBatchQuery().type(org.camunda.bpm.engine.batch.Batch_Fields.TYPE_HISTORIC_DECISION_INSTANCE_DELETION).singleResult();
		...
		historicDecisionInstances = engineRule.HistoryService.createHistoricDecisionInstanceQuery().list();
