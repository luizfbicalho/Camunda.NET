 ...
 */
namespace org.camunda.bpm.qa.upgrade.scenarios770
{
...
//	import static org.junit.Assert.assertEquals;
	...
	[ScenarioUnderTest("CreateProcessInstanceWithVariableScenario"), Origin("7.7.0")]
...
//ORIGINAL LINE: @ScenarioUnderTest("initProcessInstance.1") @Test public void testCreateProcessInstanceWithVariable()
	  ...
	  {
		// then
		ProcessInstance processInstance = engineRule.RuntimeService.createProcessInstanceQuery().processInstanceBusinessKey("process").singleResult();
		IList<HistoricVariableInstance> variables = engineRule.HistoryService.createHistoricVariableInstanceQuery().processInstanceId(processInstance.Id).list();
		assertEquals(1, variables.Count);
		assertEquals("foo", variables[0].Name);
