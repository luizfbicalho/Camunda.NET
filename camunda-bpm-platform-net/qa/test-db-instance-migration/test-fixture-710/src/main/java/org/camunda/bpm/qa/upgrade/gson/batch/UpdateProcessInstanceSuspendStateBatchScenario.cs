...
//ORIGINAL LINE: @Deployment public static String deploy()
	  ...
	  private class ScenarioSetupAnonymousInnerClass : ScenarioSetup
	  {
		  public void execute(ProcessEngine engine, string scenarioName)
		  {
			...
			for (int i = 0; i < 10; i++)
			{
			  ProcessInstance processInstance = engine.RuntimeService.startProcessInstanceByKey("oneTaskProcess_710", "UpdateProcessInstanceSuspendStateBatchScenario");

			  processInstanceIds.Add(processInstance.Id);
			}

			engine.RuntimeService.updateProcessInstanceSuspensionState().byProcessInstanceIds(processInstanceIds).suspendAsync();
		  }
	  }
	}
