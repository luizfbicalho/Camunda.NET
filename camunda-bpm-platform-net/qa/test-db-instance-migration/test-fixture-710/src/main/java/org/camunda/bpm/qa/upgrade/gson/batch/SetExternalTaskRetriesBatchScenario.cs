...
//ORIGINAL LINE: @Deployment public static String deploy()
	  ...
	  private class ScenarioSetupAnonymousInnerClass : ScenarioSetup
	  {
		  public void execute(ProcessEngine engine, string scenarioName)
		  {
			...
			for (int i = 0; i < 10; i++)
			{
			  string processInstanceId = engine.RuntimeService.startProcessInstanceByKey("externalTaskProcess_710").Id;

			  string externalTaskId = engine.ExternalTaskService.createExternalTaskQuery().processInstanceId(processInstanceId).singleResult().Id;

			  externalTaskIds.Add(externalTaskId);
			}

			engine.ExternalTaskService.setRetriesAsync(externalTaskIds, null, 22);
		  }
	  }
	}
