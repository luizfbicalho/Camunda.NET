...
//ORIGINAL LINE: @Deployment public static String deploy()
	  ...
	  private class ScenarioSetupAnonymousInnerClass : ScenarioSetup
	  {
		  public void execute(ProcessEngine engine, string scenarioName)
		  {
			...
			for (int i = 0; i < 10; i++)
			{
			  ProcessInstance processInstance = engine.RuntimeService.startProcessInstanceByKey("oneTaskProcessRestart_710", "RestartProcessInstanceBatchScenario");

			  processDefinitionId = processInstance.ProcessDefinitionId;

			  processInstanceIds.Add(processInstance.Id);

			  string taskId = engine.TaskService.createTaskQuery().processDefinitionKey("oneTaskProcessRestart_710").processInstanceBusinessKey("RestartProcessInstanceBatchScenario").singleResult().Id;

			  engine.TaskService.complete(taskId);
			}

			engine.RuntimeService.restartProcessInstances(processDefinitionId).startBeforeActivity("theTask").processInstanceIds(processInstanceIds).skipCustomListeners().skipIoMappings().withoutBusinessKey().executeAsync();
		  }
	  }
	}
