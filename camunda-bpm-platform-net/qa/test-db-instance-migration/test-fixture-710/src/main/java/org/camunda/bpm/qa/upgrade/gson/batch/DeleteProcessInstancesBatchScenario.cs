...
//ORIGINAL LINE: @Deployment public static String deploy()
	  ...
	  private class ScenarioSetupAnonymousInnerClass : ScenarioSetup
	  {
		  public void execute(ProcessEngine engine, string scenarioName)
		  {
			...
			for (int i = 0; i < 10; i++)
			{
			  engine.RuntimeService.startProcessInstanceByKey("oneTaskProcess_710", "DeleteProcessInstancesBatchScenario");
			}

			IList<string> processInstanceIds = new List<string>();

			IList<ProcessInstance> processInstances = engine.RuntimeService.createProcessInstanceQuery().processDefinitionKey("oneTaskProcess_710").list();
			...
			engine.RuntimeService.deleteProcessInstancesAsync(processInstanceIds, null);
		  }
	  }
	}
