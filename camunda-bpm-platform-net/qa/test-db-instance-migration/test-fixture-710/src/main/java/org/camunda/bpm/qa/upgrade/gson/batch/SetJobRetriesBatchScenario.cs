...
//ORIGINAL LINE: @Deployment public static String deploy()
	  ...
	  private class ScenarioSetupAnonymousInnerClass : ScenarioSetup
	  {
		  public void execute(ProcessEngine engine, string scenarioName)
		  {
			for (int i = 0; i < 10; i++)
			{
			  engine.RuntimeService.startProcessInstanceByKey("oneTaskProcessAsync_710");
			}

			IList<string> jobIds = new List<string>();

			IList<Job> jobs = engine.ManagementService.createJobQuery().processDefinitionKey("oneTaskProcessAsync_710").list();
			...
			engine.ManagementService.setJobRetriesAsync(jobIds, 22);
		  }
	  }
	}
