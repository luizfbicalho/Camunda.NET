 ...
 */
namespace org.camunda.bpm.integrationtest.functional.ejb.remote
{
	using ProcessInstance = org.camunda.bpm.engine.runtime.ProcessInstance;
	using BusinessInterface = org.camunda.bpm.integrationtest.functional.ejb.remote.bean.BusinessInterface;
	using RemoteSLSBClientDelegateBean = org.camunda.bpm.integrationtest.functional.ejb.remote.bean.RemoteSLSBClientDelegateBean;
	using RemoteSLSBean = org.camunda.bpm.integrationtest.functional.ejb.remote.bean.RemoteSLSBean;
...
//ORIGINAL LINE: @RunWith(Arquillian.class) public class RemoteSLSBInvocationTest extends org.camunda.bpm.integrationtest.util.AbstractFoxPlatformIntegrationTest
	public class RemoteSLSBInvocationTest : AbstractFoxPlatformIntegrationTest
	{
		[Deployment(name:"pa", order:2)]
		public static WebArchive processArchive()
		{
		return initWebArchiveDeployment().addClass(typeof(RemoteSLSBClientDelegateBean)).addClass(typeof(BusinessInterface)).addAsResource("org/camunda/bpm/integrationtest/functional/ejb/remote/RemoteSLSBInvocationTest.testInvokeBean.bpmn20.xml");
		}

	  [Deployment(order:1)]
	  public static WebArchive delegateDeployment()
	  {
		WebArchive webArchive = ShrinkWrap.create(typeof(WebArchive), "service.war").addAsLibraries(DeploymentHelper.EjbClient).addAsWebInfResource(EmptyAsset.INSTANCE, "beans.xml").addClass(typeof(AbstractFoxPlatformIntegrationTest)).addClass(typeof(RemoteSLSBean)).addClass(typeof(BusinessInterface)); // the business interface
