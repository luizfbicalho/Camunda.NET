...
namespace org.camunda.bpm.integrationtest.functional.@event
{
	...
	using CdiEventSupportProcessApplication = org.camunda.bpm.integrationtest.functional.@event.beans.CdiEventSupportProcessApplication;
	using EventObserverCdiBean = org.camunda.bpm.integrationtest.functional.@event.beans.EventObserverCdiBean;
...
//ORIGINAL LINE: @RunWith(Arquillian.class) public class CdiProcessApplicationEventSupportTest extends org.camunda.bpm.integrationtest.util.AbstractFoxPlatformIntegrationTest
	public class CdiProcessApplicationEventSupportTest : AbstractFoxPlatformIntegrationTest
	{
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
...
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: @Test public void testEventListener()
	  public virtual void testEventListener()
	  {
		...
		Assert.assertEquals(6, listenerInvocationCount.Value);
		...
		listenerInvocationCount = (int?) runtimeService.getVariable(processInstance.Id, ExecutionListenerProcessApplication.LISTENER_INVOCATION_COUNT);
		Assert.assertEquals(7, listenerInvocationCount.Value);
	  }
