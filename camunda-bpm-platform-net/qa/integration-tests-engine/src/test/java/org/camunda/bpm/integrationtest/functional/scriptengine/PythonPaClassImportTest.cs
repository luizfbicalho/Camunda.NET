...
//ORIGINAL LINE: @RunWith(Arquillian.class) public class PythonPaClassImportTest extends org.camunda.bpm.integrationtest.util.AbstractFoxPlatformIntegrationTest
	public class PythonPaClassImportTest : AbstractFoxPlatformIntegrationTest
	{

	  public const string SCRIPT_WITH_IMPORT = "from org.camunda.bpm.integrationtest.functional.scriptengine.classes import CustomClass\n"
		+ "execution.setVariable('greeting', CustomClass().greet())";

	  public const string JYTHON_MODULE_DEPENDENCY = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"
	  ...
	  {
		return initWebArchiveDeployment("pa1.war").addAsWebInfResource(new StringAsset(JYTHON_MODULE_DEPENDENCY),"jboss-deployment-structure.xml").addAsResource(createScriptTaskProcess("python", "", "process1"), "process1.bpmn20.xml");
	  }

	  [Deployment(name:"pa2")]
	  public static WebArchive createProcessApplication2()
	  {
		return initWebArchiveDeployment("pa2.war").addClass(typeof(CustomClass)).addAsWebInfResource(new StringAsset(JYTHON_MODULE_DEPENDENCY),"jboss-deployment-structure.xml").addAsResource(createScriptTaskProcess("python", SCRIPT_WITH_IMPORT, "process2"), "process2.bpmn20.xml");
	  }
