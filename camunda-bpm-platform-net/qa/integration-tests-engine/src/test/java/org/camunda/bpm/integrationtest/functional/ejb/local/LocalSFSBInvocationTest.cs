...
namespace org.camunda.bpm.integrationtest.functional.ejb.local
{
	...
	using LocalSFSBClientDelegateBean = org.camunda.bpm.integrationtest.functional.ejb.local.bean.LocalSFSBClientDelegateBean;
	using LocalSFSBean = org.camunda.bpm.integrationtest.functional.ejb.local.bean.LocalSFSBean;
...
//ORIGINAL LINE: @RunWith(Arquillian.class) public class LocalSFSBInvocationTest extends org.camunda.bpm.integrationtest.util.AbstractFoxPlatformIntegrationTest
	public class LocalSFSBInvocationTest : AbstractFoxPlatformIntegrationTest
	{
		[Deployment(name:"pa", order:2)]
		public static WebArchive processArchive()
		{
		return initWebArchiveDeployment().addClass(typeof(LocalSFSBClientDelegateBean)).addAsResource("org/camunda/bpm/integrationtest/functional/ejb/local/LocalSFSBInvocationTest.testInvokeBean.bpmn20.xml").addAsWebInfResource("org/camunda/bpm/integrationtest/functional/ejb/local/jboss-deployment-structure.xml","jboss-deployment-structure.xml");
		}

	  [Deployment(order:1)]
	  public static WebArchive delegateDeployment()
	  {
		WebArchive webArchive = ShrinkWrap.create(typeof(WebArchive), "service.war").addAsLibraries(DeploymentHelper.EjbClient).addAsWebInfResource(EmptyAsset.INSTANCE, "beans.xml").addClass(typeof(AbstractFoxPlatformIntegrationTest)).addClass(typeof(LocalSFSBean)).addClass(typeof(BusinessInterface)); // the business interface
