...
namespace org.camunda.bpm.integrationtest.functional.ejb.remote
{
	...
	using RemoteSingletonBean = org.camunda.bpm.integrationtest.functional.ejb.remote.bean.RemoteSingletonBean;
	using RemoteSingletonBeanClientDelegateBean = org.camunda.bpm.integrationtest.functional.ejb.remote.bean.RemoteSingletonBeanClientDelegateBean;
...
//ORIGINAL LINE: @RunWith(Arquillian.class) public class RemoteSingletonBeanInvocationTest extends org.camunda.bpm.integrationtest.util.AbstractFoxPlatformIntegrationTest
	public class RemoteSingletonBeanInvocationTest : AbstractFoxPlatformIntegrationTest
	{
		[Deployment(name:"pa", order:2)]
		public static WebArchive processArchive()
		{
		return initWebArchiveDeployment().addClass(typeof(RemoteSingletonBeanClientDelegateBean)).addClass(typeof(BusinessInterface)).addAsResource("org/camunda/bpm/integrationtest/functional/ejb/remote/RemoteSingletonBeanInvocationTest.testInvokeBean.bpmn20.xml");
		}

	  [Deployment(order:1)]
	  public static WebArchive delegateDeployment()
	  {
		WebArchive webArchive = ShrinkWrap.create(typeof(WebArchive), "service.war").addAsLibraries(DeploymentHelper.EjbClient).addAsWebInfResource(EmptyAsset.INSTANCE, "beans.xml").addClass(typeof(AbstractFoxPlatformIntegrationTest)).addClass(typeof(RemoteSingletonBean)).addClass(typeof(BusinessInterface)); // the business interface
