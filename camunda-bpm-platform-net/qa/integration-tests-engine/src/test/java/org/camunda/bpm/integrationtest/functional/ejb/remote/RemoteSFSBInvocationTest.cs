...
namespace org.camunda.bpm.integrationtest.functional.ejb.remote
{
	...
	using RemoteSFSBClientDelegateBean = org.camunda.bpm.integrationtest.functional.ejb.remote.bean.RemoteSFSBClientDelegateBean;
	using RemoteSFSBean = org.camunda.bpm.integrationtest.functional.ejb.remote.bean.RemoteSFSBean;
...
//ORIGINAL LINE: @RunWith(Arquillian.class) public class RemoteSFSBInvocationTest extends org.camunda.bpm.integrationtest.util.AbstractFoxPlatformIntegrationTest
	public class RemoteSFSBInvocationTest : AbstractFoxPlatformIntegrationTest
	{
		[Deployment(name:"pa", order:2)]
		public static WebArchive processArchive()
		{
		return initWebArchiveDeployment().addClass(typeof(RemoteSFSBClientDelegateBean)).addClass(typeof(BusinessInterface)).addAsResource("org/camunda/bpm/integrationtest/functional/ejb/remote/RemoteSFSBInvocationTest.testInvokeBean.bpmn20.xml");
		}

	  [Deployment(order:1)]
	  public static WebArchive delegateDeployment()
	  {
		WebArchive webArchive = ShrinkWrap.create(typeof(WebArchive), "service.war").addAsLibraries(DeploymentHelper.EjbClient).addAsWebInfResource(EmptyAsset.INSTANCE, "beans.xml").addClass(typeof(AbstractFoxPlatformIntegrationTest)).addClass(typeof(RemoteSFSBean)).addClass(typeof(BusinessInterface)); // the business interface
