...
//	import static org.jboss.@as.server.deployment.Attachments.MODULE;
	...
	public class ProcessApplicationDeploymentProcessor : DeploymentUnitProcessor
	{
	  ...
	  public const int PRIORITY = 0x0001; // this can happen early in the phase
...
//ORIGINAL LINE: final org.jboss.msc.service.ServiceName noViewStartService = org.camunda.bpm.container.impl.jboss.service.ServiceNames.forNoViewProcessApplicationStartService(moduleName);
		...
		ServiceBuilder<ProcessApplicationStartService> serviceBuilder = phaseContext.ServiceTarget.addService(paStartServiceName, paStartService).addDependency(phaseContext.PhaseServiceName).addDependency(ServiceNames.forBpmPlatformPlugins(), typeof(BpmPlatformPlugins), paStartService.PlatformPluginsInjector).addDependencies(deploymentServiceNames).setInitialMode(ServiceController.Mode.ACTIVE);

		if (phaseContext.ServiceRegistry.getService(ServiceNames.forDefaultProcessEngine()) != null)
		{
		  serviceBuilder.addDependency(ServiceNames.forDefaultProcessEngine(), typeof(ProcessEngine), paStartService.DefaultProcessEngineInjector);
		}
		if (paViewServiceName != null)
		{
