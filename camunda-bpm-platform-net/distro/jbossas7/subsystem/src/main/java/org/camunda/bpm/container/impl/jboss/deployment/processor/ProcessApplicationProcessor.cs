...
namespace org.camunda.bpm.container.impl.jboss.deployment.processor
{
	...
	using WarMetaData = org.jboss.@as.web.deployment.WarMetaData;
	using WebAttachments = org.jboss.@as.web.deployment.WebAttachments;
	using WebComponentDescription = org.jboss.@as.web.deployment.component.WebComponentDescription;
	using WebComponentInstantiator = org.jboss.@as.web.deployment.component.WebComponentInstantiator;
...
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final org.jboss.as.web.deployment.WarMetaData warMetaData = deploymentUnit.getAttachment(org.jboss.as.web.deployment.WarMetaData.ATTACHMENT_KEY);
		...
		else
		{
		  ...
		  if (servletProcessApplications.Contains(paClassInfo))
		  {
			...
			if (!isListener)
			{
			  ...
			  deploymentUnit.getAttachment(WebAttachments.WEB_COMPONENT_INSTANTIATORS).put(paWebComponent.ComponentClassName, new WebComponentInstantiator(deploymentUnit, paWebComponent));
